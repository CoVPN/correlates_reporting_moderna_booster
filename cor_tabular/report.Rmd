...
base::load(here::here("cor_tabular", "output", attr(config,"config"), d))
  ...
  if (nrow(tab)==1){
    tab <- data.frame(t(sapply(tab,function(x)gsub("%", "\\\\%", x))),
                    check.names = FALSE)
  }else{
  ...
  colnames(tab) <- gsub("-", "\\-", colnames(tab))
  colnames(tab) <- gsub("Non-Naive", "Non\\-Naive",
                        colnames(tab))
  ...
  for (j in jloop) {
      ...
      tabjk <- kable(if (is.na(deselect)) {tab.j} else {tab.j %>% select(-!!deselect)},
        ...
        kable_styling(latex_options = c("repeat_header", "scale_down"), 
                      ...
                      font_size = ifelse(is.null(tlf_labels[["font_size"]]), 9.5, tlf_labels[["font_size"]])) %>%
