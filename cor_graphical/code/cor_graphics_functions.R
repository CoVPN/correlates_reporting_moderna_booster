...
#' a function to define descriptive statistics by group
...
get_desc_by_group <- function(data, 
    ...
    data[which(data$time=="DD1"), "wt"]=data[which(data$time=="DD1"), "wt.DD1"]
    ...
    dat_stats <-
        ...
        mutate(counts = n(),
               ...
               RespRate = ifelse(denom!=0, paste0(round(num/denom*100, 1),"%"), ""), # RespRate at Delta timepoints will be ""
               min = min(value),
    ...
    p1 <- dat %>%
        ...
        purrr::map(function(d){
            ggplot(data = d, aes(x = cohort_event, y = value)) +
                ...
                # Whisker: Q3 + 1.5 IQR
                geom_text(aes(label = ifelse(RespRate!="","Rate",""), x = 0.4, y = 6.3), hjust = 0, color = "black", size = panel.text.size, check_overlap = TRUE) +
                geom_text(aes(x = cohort_event, label = RespRate, y = 6.3), color = "black", size = panel.text.size, check_overlap = TRUE) +
    ...
    p2 <- dat %>%
        ...
        purrr::map(function(d){
            ggplot(data = d, aes_string(x = x.var, y = "value", color = "cohort_event")) +
                ...
                geom_text(aes(label = ifelse(RespRate!="","Rate",""), x = 0.4, y = 6.5), hjust = 0, color = "black", size = panel.text.size, check_overlap = TRUE) +
